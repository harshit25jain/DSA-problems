...
The key observation is that by comparing nums[mid] and nums[mid+1], I can determine the direction of the slope.
If nums[mid] > nums[mid+1], the slope is decreasing. 
This means we are either standing on a peak or have already crossed one, so a peak must exist at mid or somewhere on the left side.
